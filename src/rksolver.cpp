...
// Assumptions:
//   None
inline int growfac(int x) { return (x * 17) / 10; }
...
int growArrays(std::vector<double> &arrays, unsigned int narrays, unsigned int oldsize,
               unsigned int newsize) {
  std::vector<double> newarrays(narrays * newsize);

  size_t j = 0;
  for (size_t n = 0; n < narrays; n++) {
    auto ulim = n * newsize + oldsize;
    for (size_t i = n * newsize; i < ulim; i++, j++) {
      newarrays[i] = arrays[j];
    }
  }
  arrays = std::move(newarrays);
  return newsize;
}
...
inline double maxnorm(const std::vector<double> vector, unsigned int length) {
  double norm = fabs(vector[0]), tst;
  size_t i = 1;

  do {
    if ((tst = fabs(vector[i++])) > norm) norm = tst;
  } while (i < length);
  return norm;
}
...
// Assumptions:
//   The pointer, vector, points to an array of the given length.
// inline double norm(const double *vector, unsigned int length) { return maxnorm(vector, length); }
...
// inline unsigned int findIndex(const double *array, double val, unsigned int size) {
...
//   while (ri - li > 1) {
//     mib = (ri + li) / 2;
//     mir = ((int)((2.0 * val - (array[li] + array[ri])) * (double)(ri - li) /
//                  (array[ri] - array[li])) +
//            ri + li) /
//           2;
//     if (mib < mir) {
...
//   the calculation.
double Embedded_Runge_Kutta_Pair_ODE_Solver::initializeStep(double step, double *vars,
                                                            double (**derivFuncs)(double *, double),
                                                            unsigned int nDepVars, double tolerance,
                                                            short int flag) {
  double err;
  ...
  //       // magnitude larger.
  //       return initializeStep(step * MAG, vars, derivFuncs, nDepVars, tolerance, 1);
  //     }
  //   } else {
...
inline double Embedded_Runge_Kutta_Pair_ODE_Solver::doStep(const double step,
                                                           double (**derivFuncs)(double *, double),
                                                           unsigned int nDepVars, const double ti,
                                                           const double *initConds,
                                                           unsigned int initCondSep, double *xVals,
                                                           unsigned int xSep) {
  //   int xi, si, si2, xici, xvi;
  //   double tmid, derivVals[nDepVars][k_stages], xqvals[nDepVars], delta[nDepVars];
...
void Embedded_Runge_Kutta_Pair_ODE_Solver::doQuickStep(double step,
                                                       double (**derivFuncs)(double *, double),
                                                       unsigned int nDepVars, const double ti,
                                                       const double *initConds, double *xVals) {
  int xi1, si1, si2;
...
// inline double Embedded_Runge_Kutta_Pair_ODE_Solver::newStep(double step, double err,
//                                                             double tolerance) {
...
// Assumptions:
//   err and tolerance are positive numbers.
// inline double Embedded_Runge_Kutta_Pair_ODE_Solver::newStepLim(double step, double err,
//                                                                double tolerance) {
...
//   if (tdiff > 0) {
//     if (tVal < indepVar[0] - tdiff || tVal > 2 * indepVar[nSteps - 1] - indepVar[nSteps - 2]) {
...
//     double ti, double tf, const double *initConds, double (**derivFuncs)(double *, double),
//     unsigned int nDepVars, double tolerance, double stepSize, unsigned int initLen) {
...
//   }
//   vars[0] = ti;
//   for (iter = 0; iter < nDepVars; iter++) vars[(iter + 1) * vlen] = initConds[iter];
...
//   stepSize = initializeStep(stepSize, vars, derivFuncs, nDepVars, tolerance, 0);
...
//     if (nSteps >= vlen) vlen = growArrays(vars, nDepVars + 1, vlen, growfac(vlen));
//     err = doStep(stepSize, derivFuncs, nDepVars, vars[nSteps - 1], &vars[vlen + nSteps - 1],
//     vlen,
//                  &vars[vlen + nSteps], vlen);
//     while (err > tolerance) {
...
//     }
//   }
//   for (iter = 0; iter < nSteps; iter++) indepVar[iter] = vars[iter];
//   for (iter = 0; iter < nDepVars; iter++)
...
// Embedded_Runge_Kutta_Pair_ODE_Solver::Embedded_Runge_Kutta_Pair_ODE_Solver(
//     double ti, double tf, const double *initConds, double (**derivFuncs)(double *, double),
//     unsigned int nDepVars, int dokeepsoln, double tolerance, double stepSize,
//     unsigned int initLen) {
//   double *vars, err;
//   int iter;

//   if (dokeepsoln != 0) {
//     Embedded_Runge_Kutta_Pair_ODE_Solver(ti, tf, initConds, derivFuncs, nDepVars, tolerance,
//                                          stepSize, initLen);
//     return;
//   }
...
//   vars[0] = ti;
//   for (iter = 0; iter < nDepVars; iter++) vars[(iter + 1) * 2] = initConds[iter];
//   stepSize = initializeStep(stepSize, vars, derivFuncs, nDepVars, tolerance, 0);
//   while (vars[1] < tf) {
//     // make the solution the new initial conditions.
//     for (iter = 0; iter < 2 * (nDepVars + 1); iter += 2) vars[iter] = vars[iter + 1];
//     // do the next step.
//     err = doStep(stepSize, derivFuncs, nDepVars, vars[0], &vars[2], 2, &vars[3], 2);
//     while (err > tolerance) {
//       stepSize = newStep(stepSize, err, tolerance);
//       err = doStep(stepSize, derivFuncs, nDepVars, vars[0], &vars[2], 2, &vars[3], 2);
//     }
...
//   if (xIndex >= nVars) {
//     cerr << "invalid independent variable " << xIndex << " in call to evalX." << endl;
//     errno = ERANGE;
//     return 0;
//   }
...
//   for (iter = 0; iter < nVars; iter++) initConds[iter] = depVars[iter][titer];
